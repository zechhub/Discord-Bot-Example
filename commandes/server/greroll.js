...
message.channel.messages.fetch(db.giveaway.last).then(m => {
    if(!m) return message.channel.send(`\`${getNow().time}\` :x: The last giveaway was not found, try \`.greroll <message id>\``);
    winner = m.reactions.cache.get("🎉").users.cache.filter((u) => !u.bot).random()
    message.channel.send(`:tada: The new winner is ${winner}! Congratulations!`)
})
    } else {
