...
module.exports = async (client, member) => {
    ...
    if(!str_chan) return;
str_chan.send({embed:{ description: `**${member.user.username}**#${member.user.discriminator} (\`${member.user.id}\`) have leave the server! \n **Roles:** \`\`\`${member.roles.cache.map(r => r.name).join(", ")}\`\`\``, color: 3553599, author: { name: "👋 Member Lost" }, footer: { text: `🕙 ${getNow().time}` } }}) 
