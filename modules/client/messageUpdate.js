...
if (err) return;
    ...
    str_chan.send({embed:{ description: `**${oldMessage.author.username}**#${oldMessage.author.discriminator} (\`${oldMessage.author.id}\`) edited his message in [\`${oldMessage.channel.name}\`](https://discord.com/channels/${oldMessage.guild.id}/${oldMessage.channel.id}) \n  **Avant**: \`\`\`${oldMessage.content}\`\`\` **Après:** \`\`\`${newMessage.content}\`\`\``, color: 3553599, author: { name: "📝 Editation" }, footer: { text: `🕙 ${getNow().time}` } }}) 
